...
def alterLabel(imgArray, lblArray):
    ...
    lblFinal = np.zeros_like(lblArray)   # 先创建一个与 lblArray 同形状、同 dtype 的零矩阵, 用来存放 最终净化后的标签。
    B, G, R = imgArray[:, :, 0], imgArray[:, :, 1], imgArray[:, :, 2]
    BMask, GMask, RMask = B == 255, G == 255, R == 255
    # mask == 1  → 线框之外的所有纯白像素（房间/室外）
    # mask == 0  → 线 (墙/栏杆/门/窗) 本身
    mask = np.logical_and(BMask, GMask)  # 0表示了被划分出去的边界
    mask = np.logical_and(mask, RMask).astype(np.uint8)  # 只有 B=G=R=255 的像素，mask 才是 1（True）
    # 利用mask，寻找联通区域
...
def worker(filePth, imgSaveDir, labelSaveDir):
    ...
    pointsBdaryIdx = []  # 存放外框各点的“顶点编号”
    newVsNum = 0  # 统计本轮新增了多少个顶点
